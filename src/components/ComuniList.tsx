...
export default function ComuniList() {
  const { setFiltroZona, filtroZona } = useAppStore();

  const handleComuneClick = (comune: string) => {
    if (filtroZona === comune) {
      setFiltroZona('');
    } else {
      setFiltroZona(comune);
    }
  };
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm border">
      ...
      <p className="text-xs text-gray-500 mt-3 text-center sm:text-left">
        Clicca su un comune per filtrare i professionisti
      </p>
    </div>
  );
} 